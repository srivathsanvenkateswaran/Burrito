...
function daysSinceBody(direction: "declines" | "gains", asset: ChartPageAsset) {
...
export default function ChartBody({ slug, asset }: { slug: string; asset: ChartPageAsset }) {
  ...
  switch (slug) {
    ...
    case "vs-benchmark": {
      ...
      if (!benchId) {
        return (
          <p className="text-sm text-muted">No benchmark is configured for {asset.symbol}.</p>
        );
      }
      const benchSymbol =
      ...
      if (common.length === 0) {
        return (
          <p className="text-sm text-muted">
            No overlapping history between {asset.symbol} and {benchSymbol} yet.
          </p>
        );
      }
    ...
    case "ytd-roi":
      return <YtdRoiChart data={loadYtdRoi(asset.id)} showCryptoCyclePresets={asset.class === "crypto"} />;
    ...
    case "days-since-percentage-decline":
      return daysSinceBody("declines", asset);
    case "days-since-percentage-gain":
      return daysSinceBody("gains", asset);
    ...
    case "roi-after-halving": {
      ...
      if (!eventRoi.halvings) {
        return (
          <p className="text-sm text-muted">
            Not available for {asset.symbol} — halving-cycle ROI is Bitcoin-only.
          </p>
        );
      }
      return (
    ...
    case "equity-risk-dashboard": {
      ...
      return <AssetTable assets={sorted} hideMcap={hideMcap} />;
    }
    case "heatmap":
    ...
    case "cross-asset-correlations": {
      ...
      if (!cross) {
        return (
          <p className="text-sm text-muted">Cross-asset correlations have not been computed yet.</p>
        );
      }
      ...
      return (
        <div className="space-y-6">
          <CorrelationMatrix ids={cross.ids} matrix={cross.matrix} />
          {pairs.length > 0 && (
            <MultiSeriesChart
    ...
    case "crypto-vs-equities": {
      ...
      if (!cross) {
        return (
          <p className="text-sm text-muted">Cross-asset comparison has not been computed yet.</p>
        );
      }
      const colors = ["#e6a144", "#8ba7c9", "#82b57a", "#de6b5a", "#b391bf", "#7fb5a8", "#c07a4a"];
      const seriesFor = (baseRows: ({ date: string } & Record<string, number | null>)[]) => {
