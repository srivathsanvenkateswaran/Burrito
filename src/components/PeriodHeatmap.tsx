/** Year × period heatmap (months, quarters, …) of % returns. */
export interface PeriodReturn {
  year: number;
  period: number; // 1-based column index
  pct: number;
}
...
export default function PeriodHeatmap({
  ...
  for (const r of returns) {
    ...
    byYear.get(r.year)!.set(r.period, r.pct);
  }
      ...
      <table className="w-full border-separate border-spacing-0.5 font-mono text-xs">
        <thead>
          <tr>
            ...
            {columns.map((c) => (
              <th key={c} className="px-1 py-1 text-right font-medium text-muted">
                {c}
              </th>
            ))}
          </tr>
